...
if($_POST['goods_brand']){//원재료 수정
	...
	$supply_company=$_POST['supply_company'];//거래처명
    ...
    //원재료 정보 수정
    /*stock테이블 */
		$stock_edit_query="UPDATE stock
		                  SET goods_brand='$goods_brand', sort='$sort', goods_name='$goods_name', goods_size=$goods_size, supply_company='$supply_company' 
					      WHERE index_num=$index_num";
		$stock_edit_result=mysqli_query($conn,$stock_edit_query);
	//END 원재료 정보 수정
	...
	echo "<meta charset='utf-8'><script>alert('원재료가 수정 되었습니다.');
...
//END 원재료수정
	...
	if($stock_division['division']=="사용"){//원재료 사용 중단할 경우
   ...
   /*원재료 사용중단*/
	...
	echo "<meta charset='utf-8'><script>alert('원재료가 사용 중단 되었습니다.');
	window.location.replace('http://ccit.cafe24.com/CTSMS/information/stock.php');
	</script>";
	...
	echo "<meta charset='utf-8'><script>alert('원재료가 사용 허용 되었습니다.');
	window.location.replace('http://ccit.cafe24.com/CTSMS/information/stock.php');
	</script>";
