...
if($_POST['hidden_index']){
	...
	for($i=0; $i<count($_POST['date']); $i++){
		...
		$stock_collection_query="INSERT INTO stock_collection
					  ...
					  VALUES
					  ($sp_num,'수금','$date[$i]',$price[$i],'$sort[$i]','$account[$i]','$blank[$i]')";
	...
	for($j=0; $j<count($_POST['collection_index_num']); $j++){
		...
		$collection_price[$j]=$_POST['collection_price'][$j];//수금액
	...
	/*stock_sp 수정*/
	 ...
	 echo "<meta charset='utf-8'><script>alert('수금 등록이 되었습니다.');
		window.location.replace('http://ccit.cafe24.com/CTSMS/information/sale_collection.php');
		</script>";
}
