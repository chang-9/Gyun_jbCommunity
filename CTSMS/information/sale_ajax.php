...
$stock_sp_joint_query="SELECT *
              FROM stock_sp_joint
			  WHERE sp_index=$index_num AND state NOT LIKE '재고량 부족' AND del_state NOT LIKE 'y' AND count NOT LIKE 0
...
while($stock_sp_joint_data=mysqli_fetch_array($stock_sp_joint_result)){
    ...
    if($stock_sp_joint_data['sort']=="매출"){ 
    ...
    $query="SELECT *
	        FROM stock_sp_joint
			WHERE goods_name='$goods_name' AND sp_index=$index_num AND state LIKE '%출고%' AND del_state NOT LIKE 'y' AND count NOT LIKE 0";
...
<td id='joint_<?=$stock_sp_joint_data['index_num'];?>' data-title="주문 수량" style="text-align: center;" rowspan="<?=$tmp_cnt;?>">
<input type="text" class="form-control" onkeyup="edit_price(<?=$i?>,<?=$stock_sp_joint_data['index_num'];?>,'n','<?=$stock_sp_joint_data['goods_name'];?>','n');" id="joint_sale_count<?=$stock_sp_joint_data['index_num'];?>" name="joint_sale_count[]" value="<?=$data2['count'];?>">
...
<td id='joint_<?=$stock_sp_joint_data['index_num'];?>' data-title="제조일자" style="text-align: center;">
<?=$stock_sp_joint_data['date'];?>
</td>
<td id='joint_<?=$stock_sp_joint_data['index_num'];?>'  data-title="원가" style="text-align: center;"><?=$stock_sp_joint_data['cost_price'];?>
<input type="hidden"  class="form-control"  id='joint_cost_price<?=$stock_sp_joint_data['index_num'];?>' name="joint_cost_price[]" placeholder="" value="<?=$stock_sp_joint_data['cost_price'];?>" >
</td>
...
<td id='joint_manic_rate<?=$stock_sp_joint_data['index_num'];?>'  data-title="마진율(%)" style="text-align: center;">
<?=$manic_rate?>
</td>
<td id='joint_<?=$stock_sp_joint_data['index_num'];?>'  data-title="거래구분" style="text-align: center; ">
<select class="form-control show-tick" id="casetwo'+n2+'" onchange="edit_price(<?=$i?>,<?=$stock_sp_joint_data['index_num'];?>,'y','n','n' );" name="joint_sort[]" style="width:100px;"><option value="매출" <?if($stock_sp_joint_data['sort']=="매출"){echo "selected";}?>>매출</option><option value="반품" <?if($stock_sp_joint_data['sort']=="반품"){echo "selected";}?>>반품</option></select>
<input type="hidden" class="form-control" id="joint_original_sort<?=$stock_sp_joint_data['index_num'];?>"  name="joint_original_sort[]" placeholder="" value="<?=$stock_sp_joint_data['sort'];?>" >
</td>
<td id='joint_state<?=$stock_sp_joint_data['index_num'];?>' data-title="출고 상태" style="text-align: center;">
<?=$stock_sp_joint_data['state'];?>
</td>
...
<tr>                                           
	...
	<th style="text-align: center;"></th>
	<th style="text-align: center;" id='joint_return_sale_price' >반품계:<?=$sum_return_sale_price?></th>
	<th style="text-align: center;"></th>
	<th style="text-align: center;"></th>
	<th style="text-align: center;"></th>
</tr>
...
if($_POST['index_num2']){
	...
	?>
		...
		<select class="form-control show-tick"  onchange="edit_price2(<?=$i2?>,<?=$stock_sp_joint_data2['index_num'];?>,'y');" name="joint_sort2[]" style="width:100px;"><option value="매출" <?if($stock_sp_joint_data2['sort']=="매출"){echo "selected";}?>>매출</option><option value="반품" <?if($stock_sp_joint_data2['sort']=="반품"){echo "selected";}?>>반품</option></select>
		</td>
