...
if($_POST['start_date']){
	...
	/*$sp_date[$i]=$data['date'];//일자
                ...
                $query="SELECT *
						FROM stock_sp
						WHERE sort='매출' AND date BETWEEN '$start_date' and '$end_date+1'";
				$result=mysqli_query($conn,$query);
				 ...
				 $query2="SELECT *
						...
						WHERE division='수금' AND state='n' AND date BETWEEN '$start_date' and '$end_date+1'";
...
?>
<tr>
<td data-title="총 매출액:" style="text-align: center;"><?=number_format($sum_total_price)?></td>										
<td data-title="총 반품액:" style="text-align: center;"><?=number_format($sum_return_price)?></td>									
<td data-title="총 수금액:" style="text-align: center;"><?=number_format($sum_price)?></td>
</tr>
<?
}
?>
